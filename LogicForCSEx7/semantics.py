...
class Model(Generic[T]):
    ...
    def evaluate_term(self, term: Term,
        ...
        if is_constant(term.root):
            ...
            arguments_evalutaions = [self.evaluate_term(arg, assignment)
            for arg in term.arguments]
            return self.function_meanings[term.root][tuple(arguments_evalutaions)]
    ...
    def evaluate_formula(self, formula: Formula,
        ...
        if is_equality(formula.root):
            ...
            terms_evalutaions = [self.evaluate_term(arg, assignment)
                                 for arg in formula.arguments]

            return tuple(terms_evalutaions) in self.relation_meanings[formula.root]

        elif is_unary(formula.root):
            return not self.evaluate_formula(formula.first, assignment)

        elif is_binary(formula.root):
            f1 = self.evaluate_formula(formula.first, assignment)
            f2 = self.evaluate_formula(formula.second, assignment)

            if formula.root == AND:
                return f1 and f2
            elif formula.root == OR:
                return f1 or f2
            elif formula.root == IMPLY:
                return (f1 and f2) or not f1
            ...
            if formula.root == ALL:
                for element in self.universe:
                    ...
                    # There exists one model where formula evaluates to False
                    if not self.evaluate_formula(formula.predicate, new_assignment):
                        return False
                # for all assignments of the variable of quantification,
                # formula evaluates to True
                return True
            elif formula.root == EXISTS:
                for element in self.universe:
                    new_assignment = dict(assignment)
                    new_assignment[formula.variable] = element
                    # There exists one model where formula evaluates to True
    ...
    def is_model_of(self, formulas: AbstractSet[Formula]) -> bool:
        ...
        # Task 7.9
        for formula in formulas:
            variables = list(formula.free_variables())
            for assignment in itertools.product(self.universe, repeat=len(variables)):
                new_assignment = {variables[i]: assignment[i] for i in range(len(variables))}
                if not self.evaluate_formula(formula, new_assignment):
                    return False
        # all formulas evaluate to true
        return True
