...
def evaluate_inference(rule: InferenceRule, model: Model) -> bool:
    ...
    # Task 4.2
    # there are 2 cases:
    # case 1: if one of the assumption is incorrect, than the conclusion
    #          can be correct or not correct (True or False)
    # case 2 : if all the assumption correct(True) than the conclusion needs
    #           to be True
    bool_val_conclusion = evaluate(rule.conclusion, model)
    for formula in rule.assumptions:
        # case 1
        assumption = evaluate(formula, model)
        if not assumption:
            return True
    # case 2
    if bool_val_conclusion:
        return True
    return False
...
def is_sound_inference(rule: InferenceRule) -> bool:
    ...
    """
    # Task 4.3
    # checks that the InferenceRule holds for every possible model
